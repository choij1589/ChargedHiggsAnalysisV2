...
def loadDataset(process, mA, width, sigma):
    ...
    for i in range(args.nfold):
        events[i] = []
    source = ROOT.TFile(f"{WORKDIR}/SignalRegionStudy/samples/{args.era}/{args.channel.replace('SR', 'Skim')}/{args.masspoint}/{process}.root")
    tree = source.Get(f"{process}_Central")
...
def plotGBDTOutput(models, events_sig, events_bkg, best_threshold, improvement):
    ...
    c.SaveAs(f"{base_path}/gbdt_output.png")   
    
def updateScore(process, syst, models):
    logging.info(f"Updating GBDT score for {process} {syst}")
    f = ROOT.TFile.Open(f"{WORKDIR}/SignalRegionStudy/samples/{args.era}/{args.channel.replace('SR', 'Skim')}/{args.masspoint}/{process}.root", "UPDATE")
...
def getHist(process, mA, width, sigma, threshold=-999., syst="Central"):
    ...
    file_path = f"{WORKDIR}/SignalRegionStudy/samples/{args.era}/{args.channel.replace('SR', 'Skim')}/{args.masspoint}/{process}.root"
    hist_name = process if syst == "Central" else f"{process}_{syst}"
    hist_range = (15, mA - 5 * width - 3 * sigma, mA + 5 * width + 3 * sigma)
    tree_name = f"{process}_{syst}"
    central_name = f"{process}_Central"
...
if __name__ == "__main__":
    ...
    if args.update: updateScore(args.masspoint, "Central", models)
    central = getHist(args.masspoint, mA, width, sigma, best_threshold); f.cd(); central.Write()
    for syst in promptSysts:
        if args.update: updateScore(args.masspoint, f"{syst}Up", models)
        if args.update: updateScore(args.masspoint, f"{syst}Down", models)
    ...
    hists_pdf = []
    for i in range(100):
        if args.update: updateScore(args.masspoint, f"PDFReweight_{i}", models)
    ...
    for i in [0, 1, 2, 3, 4, 6, 8]:
        if args.update: updateScore(args.masspoint, f"ScaleVar_{i}", models)
    ...
    hists_ps = []
    for i in range(4):
        if args.update: updateScore(args.masspoint, f"PSVar_{i}", models)
    ...
    for process in ["WZ", "ZZ"]:
        if args.update: updateScore(process, "Central", models)
        hist = getHist(process, mA, width, sigma, best_threshold); data_obs.Add(hist); f.cd(); hist.Write()
        for syst in promptSysts:
            if args.update: updateScore(process, f"{syst}Up", models)
            if args.update: updateScore(process, f"{syst}Down", models)
    ...
    if args.update: updateScore("others", "Central", models)
    hist = getHist("others", mA, width, sigma, best_threshold); data_obs.Add(hist); f.cd(); hist.Write()
    for syst in promptSysts:
        if args.update: updateScore("others", f"{syst}Up", models)
        if args.update: updateScore("others", f"{syst}Down", models)
